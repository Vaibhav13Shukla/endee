...
async def process_query(
    query_request: QueryRequest, current_user: User = Depends(get_current_active_user)
):
...
async def health_check():
    """Health check that verifies Endee and MongoDB connectivity"""
    ...
    try:
        resp = await asyncio.to_thread(
            requests.get, f"{Config.ENDEE_HOST}/health", timeout=3
        )
        if resp.status_code == 200:
            health["services"]["endee"] = "connected"
        else:
            health["services"]["endee"] = "unhealthy"
            health["status"] = "degraded"
    except Exception:
        health["services"]["endee"] = "disconnected"
        health["status"] = "degraded"
